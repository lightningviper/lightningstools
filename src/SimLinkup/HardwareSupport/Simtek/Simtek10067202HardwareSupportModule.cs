...
namespace SimLinkup.HardwareSupport.Simtek
{
    ...
    //
    // Same physical gauge family as Simtek 10-0335-015 (F-16 Standby ADI),
    // hand-authored breakpoints over the ±90° pitch range capture the
    // synchro's nonlinearity at this part number's specific rotor wiring.
    // Roll is a straight 10·sin/cos.
    //
    // Editor-authored calibration mirrors Simtek100335015:
    //   - pitch sin/cos: piecewise_resolver (breakpoint table per channel)
    //   - roll  sin/cos: piecewise_resolver
    //   - OFF flag:       digital_invert
    //   - optional caged-rest random angle when OFF flag visible
    public class Simtek10067202HardwareSupportModule : HardwareSupportModuleBase, IDisposable
    {
        private static readonly ILog _log = LogManager.GetLogger(typeof(Simtek10067202HardwareSupportModule));
        ...
        private Simtek10067202HardwareSupportModuleConfig _config;
        ...
        public Simtek10067202HardwareSupportModule(Simtek10067202HardwareSupportModuleConfig config)
        {
            _config = config;
        ...
        private void ResolveAllChannels(GaugeCalibrationConfig config)
        {
            ResolvePiecewiseResolverPair(config,
                "10067202_Pitch_SIN_To_Instrument",
                "10067202_Pitch_COS_To_Instrument",
                out _pitchTransform, out _pitchSinChannel, out _pitchCosChannel);
            ResolvePiecewiseResolverPair(config,
                "10067202_Roll_SIN_To_Instrument",
                "10067202_Roll_COS_To_Instrument",
            ...
            _offFlagChannel = config != null
                ? config.FindChannel("10067202_OFF_Flag_To_Instrument")
                : null;
        ...
        private void ReloadConfig()
        {
            try
            {
                ...
                var reloaded = Simtek10067202HardwareSupportModuleConfig.Load(configFile);
        ...
        public override string FriendlyName => "Simtek P/N 10-0672-02 - Indicator - Simulated Standby Attitude";
        ...
        ~Simtek10067202HardwareSupportModule()
        {
            Dispose(false);
        }
        ...
        {
            Simtek10067202HardwareSupportModuleConfig hsmConfig = null;
            try
            {
                var hsmConfigFilePath = Path.Combine(
                    Util.CurrentMappingProfileDirectory,
                    "Simtek10067202HardwareSupportModule.config");
                hsmConfig = Simtek10067202HardwareSupportModuleConfig.Load(hsmConfigFilePath);
                if (hsmConfig != null)
                {
            ...
            return new IHardwareSupportModule[] { new Simtek10067202HardwareSupportModule(hsmConfig) };
        }
        ...
        private void UnregisterForInputEvents()
        {
            ...
            {
                try { _offFlagInputSignal.SignalChanged -= _offFlagInputSignalChangedEventHandler; }
        ...
        private void UpdatePitchOutputValues()
        {
            ...
            // Hardcoded fallback — the original 10-0672-02 spec table
            // (per-10° bands of sin/cos volts from this gauge's calibration
            // sheet). Preserved verbatim so behaviour without an editor
            // config matches the historical SimLinkup baseline.
            double pitchSinOutputValue = 0;
            double pitchCosOutputValue = 10;

            if (pitchInputDegrees > 0 && pitchInputDegrees <= 10)
            {
                pitchSinOutputValue = (pitchInputDegrees / 10) * 3.613;
                pitchCosOutputValue = 10 - ((pitchInputDegrees / 10) * 0.676);
            }
            else if (pitchInputDegrees > 10 && pitchInputDegrees <= 20)
            {
                pitchSinOutputValue = 3.613 + (((pitchInputDegrees - 10) / 10) * 3.125);
                pitchCosOutputValue = 9.324 - (((pitchInputDegrees - 10) / 10) * 1.935);
            }
            else if (pitchInputDegrees > 20 && pitchInputDegrees <= 30)
            {
                pitchSinOutputValue = 6.738 + (((pitchInputDegrees - 20) / 10) * 2.214);
                pitchCosOutputValue = 7.389 - (((pitchInputDegrees - 20) / 10) * 2.933);
            }
            else if (pitchInputDegrees > 30 && pitchInputDegrees <= 40)
            {
                pitchSinOutputValue = 8.952 + (((pitchInputDegrees - 30) / 10) * 1.006);
                pitchCosOutputValue = 4.456 - (((pitchInputDegrees - 30) / 10) * 3.536);
            }
            else if (pitchInputDegrees > 40 && pitchInputDegrees <= 50)
            {
                pitchSinOutputValue = 9.958 - (((pitchInputDegrees - 40) / 10) * 0.341);
                pitchCosOutputValue = 0.920 - (((pitchInputDegrees - 40) / 10) * 3.66);
            }
            else if (pitchInputDegrees > 50 && pitchInputDegrees <= 60)
            {
                pitchSinOutputValue = 9.617 - (((pitchInputDegrees - 50) / 10) * 1.639);
                pitchCosOutputValue = -2.740 - (((pitchInputDegrees - 50) / 10) * 3.289);
            }
            else if (pitchInputDegrees > 60 && pitchInputDegrees <= 70)
            {
                pitchSinOutputValue = 7.978 - (((pitchInputDegrees - 60) / 10) * 2.717);
                pitchCosOutputValue = -6.029 - (((pitchInputDegrees - 60) / 10) * 2.475);
            }
            else if (pitchInputDegrees > 70 && pitchInputDegrees <= 80)
            {
                pitchSinOutputValue = 5.261 - (((pitchInputDegrees - 70) / 10) * 2.717);
                pitchCosOutputValue = -8.504 - (((pitchInputDegrees - 70) / 10) * 1.5);
            }
            else if (pitchInputDegrees > 80 && pitchInputDegrees <= 90)
            {
                pitchSinOutputValue = 2.544 - (((pitchInputDegrees - 80) / 10) * 2.717);
                pitchCosOutputValue = -10 - (((pitchInputDegrees - 80) / 10) * 1.5);
            }
            else if (pitchInputDegrees > 90)
            {
                pitchSinOutputValue = 0;
            ...
            {
                pitchSinOutputValue = -3.244 + (((-pitchInputDegrees - 80) / 10) * 2.717);
                pitchCosOutputValue = -9.5 + (((-pitchInputDegrees - 80) / 10) * 1);
            }
            else if (pitchInputDegrees < -90)
            {
                pitchSinOutputValue = 0;
