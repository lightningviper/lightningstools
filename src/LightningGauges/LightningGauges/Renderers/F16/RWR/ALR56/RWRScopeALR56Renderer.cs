...
namespace LightningGauges.Renderers.F16.RWR.ALR56
{
    public class RWRScopeALR56Renderer : RWRRenderer
    {
        ...
        public override void Render(DrawingContext drawingContext)
        {
            ...
            {
                DrawTextCenteredVertical(drawingContext, "WO", 0, 0, true);
            }

            if (RwrInfoContains("ilbl"))
            {
                DrawTextCenteredVertical(drawingContext, "I", 0, 0, true);
            }

            if (RwrInfoContains("flbl"))
            {
                DrawTextCenteredVertical(drawingContext, "F", 0, 0, true);
            }

            if (RwrInfoContains("sllbl"))
            {
                DrawTextCenteredVertical(drawingContext, "SL", 0, 0, true);
            }
            else if (RwrInfoContains("slbl"))
            {
                DrawTextCenteredVertical(drawingContext, "S", 0, 0, true);
            }

            if (RwrInfoContains("llbl"))
            {
                DrawTextCenteredVertical(drawingContext, "L", 0, 0, true);
            }
