...
namespace LightningGauges.Renderers.F16.RWR.ALR69
{
    public class RWRScopeALR69Renderer : RWRRenderer
    {
        private enum RWRSymbol { NewDetection, Diamond, Hat, MissileActivity, MissileLaunch };

        private const int BASELINE_CYCLE_TIME_ALR69 = 500;
        private const int CYCLE_TIME_PER_CONTACT_ALR69 = 100;
        private int cycleduration = BASELINE_CYCLE_TIME_ALR69;
        ...
        public override void Render(DrawingContext drawingContext)
        {
            ...
            if (RwrInfoContains("onbitpattern"))
            {
                DrawLine(drawingContext, 0.0, 0.1, 0.0, 0.2);
                DrawLine(drawingContext, 0.0, -0.1, 0.0, -0.2);
                DrawLine(drawingContext, 0.1, 0.0, 0.2, 0.0);
                DrawLine(drawingContext, -0.1, 0.0, -0.2, 0.0);
                DrawLine(drawingContext, 0.1, 0.0, 0.1, 0.04);
                DrawLine(drawingContext, 0.1, 0.0, 0.1, -0.04);

                DrawSolidCircle(drawingContext, 0.4, 0.4, 0.02);
                DrawSolidCircle(drawingContext, -0.4, 0.4, 0.02);
            ...
            if (!RwrInfoContains("onsys") && !RwrInfoContains("onbit"))
            {
                ...
                {
                    DrawSolidCircle(drawingContext, 0, 0, 0.02);
                }

                DrawLine(drawingContext, 0.1, 0.0, 0.1, cycletimeflipper * 0.06);
            }

            if (RwrInfoContains("naval"))
            {
                double[,] lines = {
                                {-0.060F,-0.040F},
                                {-0.080F,0.000F},
            ...
            if (RwrInfoContains("on1bit"))
            {
                ...
                DrawTextCenter(drawingContext, "SYS SWV 0802", 0, 0.75, false);
                DrawTextCenter(drawingContext, "SELF TEST", 0, 0.75 - TextHeight(false), false);

                DrawTextCenter(drawingContext, "LAMPS ON", 0, -0.5, false);
                DrawTextCenter(drawingContext, "AUDIO ON", 0, -0.5 - TextHeight(false), false);
            }

            if (RwrInfoContains("on2bit"))
            {
                DrawTextCenter(drawingContext, "OFP V0802", 0, 0.75, false);
                DrawTextCenter(drawingContext, "EIO V0802", 0, 0.75 - TextHeight(false), false);
                DrawTextCenter(drawingContext, "FSRS V0802", 0, 0.75 - TextHeight(false) - TextHeight(false), false);

                DrawTextLeft(drawingContext, "INIT OK", -0.4, -0.22, false);
                DrawTextLeft(drawingContext, "MEMORY OK", -0.4, -0.22 - TextHeight(false), false);

                if (RwrInfoContains("baddf"))
                {
                    if (flashFaster)
                    {
                        DrawTextLeft(drawingContext, "DF BAD", -0.4, -0.22 - TextHeight(false) * 2, false);
                    }
                    else
                    {
                        DrawTextLeft(drawingContext, "DF", -0.4, -0.22 - TextHeight(false) * 2, false);
                    }
                }
                else
                {
                    DrawTextLeft(drawingContext, "DF OK", -0.4, -0.22 - TextHeight(false) * 2, false);
                }

                if (RwrInfoContains("badfsrs"))
                {
                    if (flashFaster)
                    {
                        DrawTextLeft(drawingContext, "FSRS BAD", -0.4, -0.22 - TextHeight(false) * 3, false);
                    }
                    else
                    {
                        DrawTextLeft(drawingContext, "FSRS", -0.4, -0.22 - TextHeight(false) * 3, false);
                    }
                }
                else
                {
                    DrawTextLeft(drawingContext, "FSRS OK", -0.4, -0.22 - TextHeight(false) * 3, false);
                }
            }

            if (RwrInfoContains("flbl"))
            {
                DrawTextCenteredVertical(drawingContext, "F", 0, 0, false);
            }
        ...
        {
            cycleduration = BASELINE_CYCLE_TIME_ALR69 + (InstrumentState.RwrObjectCount * CYCLE_TIME_PER_CONTACT_ALR69);
        ...
        private void DrawContact(DrawingContext drawingContext, int contact)
        {
            ...
            {
                DrawArc(drawingContext, xpos, ypos, 0.15d, 0d, -180d);
            }
        ...
        private void DrawEmitterSymbol(DrawingContext drawingContext, int symbol, double x, double y, char customSymbol, bool big)
        {
            ...
            switch (knownSymbol)
            {
                ...
                case ThreatSymbols.RWRSYM_A1:
                    DrawTextCenteredVertical(drawingContext, "A", x, y, big);
                    DrawTextCenteredVertical(drawingContext, ".", x, y - 0.04, big);
                    break;
                case ThreatSymbols.RWRSYM_A2:
                    DrawTextCenteredVertical(drawingContext, "A", x, y, big);
                    DrawTextCenteredVertical(drawingContext, ".", x + 0.02, y - 0.04, big);
                    DrawTextCenteredVertical(drawingContext, ".", x - 0.02, y - 0.04, big);
                    break;
                case ThreatSymbols.RWRSYM_A3:
                    DrawTextCenteredVertical(drawingContext, "A", x, y, big);
                    DrawTextCenteredVertical(drawingContext, ".", x + 0.03, y - 0.04, big);
                    DrawTextCenteredVertical(drawingContext, ".", x, y - 0.04, big);
                    DrawTextCenteredVertical(drawingContext, ".", x - 0.03, y - 0.04, big);
                    break;
                ...
                case ThreatSymbols.RWRSYM_PSLASH:
                    DrawTextCenteredVertical(drawingContext, "P", x, y, big);
                    DrawTextCenteredVertical(drawingContext, "|", x + 0.01, y, big);
                    break;
                ...
                case ThreatSymbols.RWRSYM_Ab:
                    DrawTextCenteredVertical(drawingContext, "A", x, y, big);
                    DrawTextCenteredVertical(drawingContext, "|", x + 0.02, y, big);
                    DrawTextCenteredVertical(drawingContext, "|", x, y, big);
                    break;
                case ThreatSymbols.RWRSYM_Ac:
                    DrawTextCenteredVertical(drawingContext, "A", x, y, big);
                    DrawTextCenteredVertical(drawingContext, "|", x - 0.01, y, big);
                    DrawTextCenteredVertical(drawingContext, "|", x + 0.1, y, big);
                    DrawTextCenteredVertical(drawingContext, "|", x + 0.03, y, big);
                    break;
                ...
                default:
                    ...
                    else if (symbol >= 65)
                    {
                        ...
                        {
                            DrawSymbol(drawingContext, RWRSymbol.Hat, x, y, big);
                            DrawTextCenteredVertical(drawingContext, ((char)symbol).ToString().ToUpperInvariant().ToUpperInvariant(), x, y - 0.04, big);
                        }
                        else
                        {
