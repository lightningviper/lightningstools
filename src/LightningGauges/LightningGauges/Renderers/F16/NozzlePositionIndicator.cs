...
namespace LightningGauges.Renderers.F16
{
    ...
    public class NozzlePositionIndicator : InstrumentRendererBase, INozzlePositionIndicator
    {
        ...
        private static void LoadImageResources()
        {
            lock (_imagesLock)
            {
                if (_background == null) _background = (Bitmap) Util.LoadBitmapFromFile(IMAGES_FOLDER_NAME + Path.DirectorySeparatorChar + NOZ_BACKGROUND_IMAGE_FILENAME);
                if (_background2 == null) _background2 = (Bitmap) Util.LoadBitmapFromFile(IMAGES_FOLDER_NAME + Path.DirectorySeparatorChar + NOZ_BACKGROUND2_IMAGE_FILENAME);
